...
if old_path and new_path and Path(old_path).exists() and Path(new_path).exists():
    ...
    with sheet_tab2:
        if has_details and not comp_details.empty:
            ...
            # --- Details charts (McKinsey style): indication-level pies + old vs new bar ---
            st.subheader("Details — charts")
            # Indication-level share: aggregate by Indication (total patients per indication)
            total_col_old = next(
                (c for c in (old_details.columns if not old_details.empty else []) if "Total" in str(c) and "12" in str(c)),
                old_details.columns[0] if not old_details.empty and len(old_details.columns) else None,
            )
            total_col_new = next(
                (c for c in (new_details.columns if not new_details.empty else []) if "Total" in str(c) and "6" in str(c)),
                new_details.columns[0] if not new_details.empty and len(new_details.columns) else None,
            )
            if total_col_old is None and not old_details.empty:
                total_col_old = [c for c in old_details.columns if pd.api.types.is_numeric_dtype(old_details[c])][0] if any(pd.api.types.is_numeric_dtype(old_details[c]) for c in old_details.columns) else None
            if total_col_new is None and not new_details.empty:
                total_col_new = [c for c in new_details.columns if pd.api.types.is_numeric_dtype(new_details[c])][0] if any(pd.api.types.is_numeric_dtype(new_details[c]) for c in new_details.columns) else None

            if total_col_old or total_col_new:
                pie_d1, pie_d2 = st.columns(2)
                with pie_d1:
                    if total_col_old and not old_details.empty:
                        ind_old = old_details.groupby(level="Indication")[total_col_old].sum()
                        ind_old = ind_old[ind_old > 0].sort_values(ascending=False)
                        if len(ind_old):
                            colors_d = [PIE_PALETTE[i % len(PIE_PALETTE)] for i in range(len(ind_old))]
                            fig_ind_old = go.Figure(go.Pie(
                                labels=ind_old.index.tolist(),
                                values=ind_old.values,
                                hole=0.45,
                                textinfo="label+percent",
                                textposition="outside",
                                textfont=PIE_TEXTFONT,
                                hovertemplate="%{label}<br>Patients: %{value:,.0f}<br>Share: %{percent}<extra></extra>",
                                marker=dict(colors=colors_d, line=PIE_MARKER_LINE),
                            ))
                            fig_ind_old.update_layout(
                                **PLOTLY_LAYOUT_PIE,
                                title=dict(text=f"{LABEL_OLD} — Indication share (12 mo)", font=CHART_TITLE_FONT, pad=PIE_TITLE_PAD),
                                height=400,
                                showlegend=True,
                            )
                            st.plotly_chart(fig_ind_old, use_container_width=True, key="details_pie_old")
                with pie_d2:
                    if total_col_new and not new_details.empty:
                        ind_new = new_details.groupby(level="Indication")[total_col_new].sum()
                        ind_new = ind_new[ind_new > 0].sort_values(ascending=False)
                        if len(ind_new):
                            colors_d = [PIE_PALETTE[i % len(PIE_PALETTE)] for i in range(len(ind_new))]
                            fig_ind_new = go.Figure(go.Pie(
                                labels=ind_new.index.tolist(),
                                values=ind_new.values,
                                hole=0.45,
                                textinfo="label+percent",
                                textposition="outside",
                                textfont=PIE_TEXTFONT,
                            ...
                            fig_ind_new.update_layout(
                                **PLOTLY_LAYOUT_PIE,
                                title=dict(text=f"{LABEL_NEW} — Indication share (6 mo)", font=CHART_TITLE_FONT, pad=PIE_TITLE_PAD),
                                height=400,
                                showlegend=True,
                            )
                            st.plotly_chart(fig_ind_new, use_container_width=True, key="details_pie_new")

            # Bar chart: Old vs New total patients by (Indication × Type)
            old_cols = [c for c in comp_display.columns if str(c).endswith("_old") and pd.api.types.is_numeric_dtype(comp_display[c])]
            new_cols = [c for c in comp_display.columns if str(c).endswith("_new") and pd.api.types.is_numeric_dtype(comp_display[c])]
            total_old_col = next((c for c in old_cols if "Total" in str(c)), old_cols[0] if old_cols else None)
            total_new_col = next((c for c in new_cols if "Total" in str(c)), new_cols[0] if new_cols else None)
            if total_old_col and total_new_col and len(comp_display) > 0:
                _chart_df = comp_display.sort_values(total_old_col, ascending=False)
                labels_detail = [f"{str(row['Indication'])[:14]} — {str(row['Type'])[:12]}" for _, row in _chart_df.iterrows()]
                fig_detail = go.Figure()
                fig_detail.add_trace(go.Bar(
                    name=LABEL_OLD,
                    x=labels_detail,
                    y=_chart_df[total_old_col].fillna(0).values,
                    marker_color=M["teal"],
                    marker_line_width=0,
                ))
                fig_detail.add_trace(go.Bar(
                    name=LABEL_NEW,
                    x=labels_detail,
                    y=_chart_df[total_new_col].fillna(0).values,
                    marker_color=M["navy"],
                    marker_line_width=0,
                ))
                fig_detail.update_layout(
                    **PLOTLY_LAYOUT,
                    ...
                    bargap=0.15,
                    bargroupgap=0.05,
                    title=dict(text="Total patients by Indication × Type — Old vs New", font=CHART_TITLE_FONT),
                    xaxis_tickangle=-40,
                    height=max(420, min(600, 80 + len(labels_detail) * 28)),
                )
                st.plotly_chart(fig_detail, use_container_width=True, key="details_bar")
                comp_display = _chart_df  # keep table order consistent with chart
